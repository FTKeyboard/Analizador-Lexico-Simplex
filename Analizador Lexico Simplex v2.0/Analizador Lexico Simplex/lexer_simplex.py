...
# Expresiones regulares para los tokens. Cada uno de estos patrones se asociará a un token en el código fuente.
...
# Regla para manejar errores léxicos. Si el analizador encuentra un carácter no reconocido,
# se invoca esta función para mostrar un mensaje de error y avanzar al siguiente carácter.
def t_error(t):
    r'.'
...
# Función que encuentra la columna del token en el texto
# Calcula la posición en la línea del token para proporcionar un informe de errores más preciso.
def find_column(token, lexdata):
    line_start = lexdata.rfind('\n', 0, token.lexpos) + 1
    return token.lexpos - line_start + 1

# Crear el lexer usando las reglas definidas
lexer = lex.lex()
